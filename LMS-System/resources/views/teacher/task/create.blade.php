...
@section("content")
    <div class="bg-[#181818] rounded-lg shadow-lg p-8 max-w-3xl mx-auto">
        <h2 class="text-3xl font-bold text-white mb-6">Create a New Task</h2>

        <form action="{{ route("subjects.tasks.store",["subject"=>$subject->id]) }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="pt-4 flex space-x-4">
                ...
                <a href="{{ route("subjects.show",["subject"=> $subject->id]) }}" class="bg-[#333] text-white px-6 py-3 rounded hover:bg-[#444] transition font-semibold">
                    Cancel
                </a>
            </div>
        </form>
